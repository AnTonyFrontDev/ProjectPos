...
export const getProducts = async () => {
    try {
        const response = await axios.get(`${URL}/Product/GetProducts?Page=1&ItemsPerPage=30`);
...
export const getProductsPaginated = async (page:number,items:number) => {
    try {
        return  await axios.get(`${URL}/Product/GetProducts?Page=${page}&ItemsPerPage=${items}`);
    } catch (error) {
        console.error('Error fetching products:', error);
    }
};
...
export const getInventoryById = async (productId: number) => {
    try {
        const response = await axios.get(`${URL}/Inventory/GetProductByProductId?id=${productId}`);
